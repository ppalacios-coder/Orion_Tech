...
def make_db(path: Path, records):
    ...
    for rec_id, (identifier, title, body) in enumerate(records, start=1):
        ...
        blob = plistlib.dumps({"req": {"titl": title, "body": body}}, fmt=plistlib.FMT_BINARY)
        conn.execute(
...
class BridgeTests(unittest.TestCase):
    ...
    def test_decodes_title_and_body(self):
        blob = plistlib.dumps({"req": {"titl": "BBVA", "subt": "Tarjeta", "body": "Compra"}},
                              fmt=plistlib.FMT_BINARY)
        self.assertEqual(nb.decode_record(blob), ("BBVA Tarjeta", "Compra"))
